...
def to_data(node_features, edge_attributes, edge_indices, graph_features, graph_labels):
    
#     node_features = node_features[:, :-1]
    node_features, features = extend_features(node_features, graph_features[0][0])
    
    
    # Use clone().detach() to avoid warnings
    weights = torch.pow(torch.sigmoid(edge_attributes.clone().detach()), -1)
    edges, weights = to_undirected(edge_indices.clone().detach(), weights)
    edges, weights = add_self_loops(edges, weights, 2)
   
    n = len(node_features)
    adj = SparseTensor(row=edges[0], col=edges[1], value=weights, sparse_sizes=(n, n))
    node_features = torch.cat([node_features, (adj.to_torch_sparse_coo_tensor() @ node_features[:, 1]).unsqueeze(1)], dim=1)
                        
    return pyg.data.Data(x=node_features, features=features, adj_t=adj, edge_index=edges, edge_attr=weights, y=graph_labels)
...
def edges_threshold(edge_index, edge_attributes, threshold=0.2):
    """
